...
module.exports = {
    verify: (token, remoteip) => {
        ...
        let url = `https://www.google.com/recaptcha/api/siteverify?secret=${config.recaptcha_secret}&response=${token}${query_remoteip}`;
            ...
            axios.post(url)
                ...
                .then((captchaResponse) => {
                    ...
                    if (typeof response !== 'undefined' 
                        ...
                        debug('reCaptcha Failed (funny business): %O', response)
                        reject('Request failed anti-abuse testing. Try reloading the page? (reCAPTCHA)');
                    }
                })
            ;
        });
